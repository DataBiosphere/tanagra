...
FROM (SELECT DISTINCT measurement_source_concept_id FROM `${omopDataset}.measurement`
      WHERE measurement_source_concept_id IS NOT NULL
        AND measurement_concept_id = 0) m
JOIN `${omopDataset}.concept` c ON m.measurement_source_concept_id = c.concept_id
