    ...
    WHERE
        t.id IN (
            SELECT
                t.entity_B_id 
            FROM
                `verily-tanagra-dev.aouSR2019q4r4_index_110623`.T_RIDS_brandIngredient_brand_ingredient AS t 
            WHERE
                t.entity_A_id IN (
                    SELECT
                        t.id 
                    FROM
                        `verily-tanagra-dev.aouSR2019q4r4_index_110623`.T_ENT_brand AS t 
                    WHERE
                        t.id = 19082059
                )
            ) LIMIT 30
