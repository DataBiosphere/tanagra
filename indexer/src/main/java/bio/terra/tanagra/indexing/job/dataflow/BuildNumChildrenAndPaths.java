...
import static bio.terra.tanagra.indexing.job.dataflow.beam.BigQueryBeamUtils.CHILD_COLUMN_NAME;
import static bio.terra.tanagra.indexing.job.dataflow.beam.BigQueryBeamUtils.ID_COLUMN_NAME;
import static bio.terra.tanagra.indexing.job.dataflow.beam.BigQueryBeamUtils.NUMCHILDREN_COLUMN_NAME;
import static bio.terra.tanagra.indexing.job.dataflow.beam.BigQueryBeamUtils.PARENT_COLUMN_NAME;
import static bio.terra.tanagra.indexing.job.dataflow.beam.BigQueryBeamUtils.PATH_COLUMN_NAME;
...
public class BuildNumChildrenAndPaths extends BigQueryIndexingJob {
  ...
  public TablePointer getTempTable() {
    ...
    return TablePointer.fromTableName(
        getEntity().getName() + "_" + hierarchyName + "_" + TEMP_TABLE_SUFFIX,
  ...
  private void writeFieldsToTempTable(boolean isDryRun) {
    ...
    PCollection<Long> allNodesPC =
        BigQueryBeamUtils.readNodesFromBQ(pipeline, selectAllIdsSql, "allNodes");
    PCollection<KV<Long, Long>> childParentRelationshipsPC =
        BigQueryBeamUtils.readChildParentRelationshipsFromBQ(pipeline, selectChildParentIdPairsSql);
  ...
  private static PCollection<KV<Long, String>> filterRootNodes(
    ...
    PCollection<Long> possibleRootNodesPC =
        BigQueryBeamUtils.readNodesFromBQ(pipeline, selectPossibleRootIdsSql, "rootNodes");
