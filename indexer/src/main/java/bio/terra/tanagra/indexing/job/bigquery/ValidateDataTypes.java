...
public class ValidateDataTypes extends BigQueryJob {
    ...
    public ValidateDataTypes(
            SZIndexer indexerConfig, Entity entity, STEntityAttributes sourceTable, ITEntityMain indexTable) {
        super(indexerConfig);
        this.entity = entity;
    ...
    public void run(boolean isDryRun) {
        ...
        Schema sourceQueryResultSchema = bigQueryExecutor.getBigQueryService().getQuerySchemaWithCaching(selectAttributesFromSourceTable.renderSQL());
        ...
        for (Attribute attribute : entity.getAttributes()) {
            ...
            LegacySQLTypeName sourceTableBQDataType = BigQueryBeamUtils.fromSqlDataType(sourceTableSchema.getSqlDataType());

            Field sourceQueryField = sourceQueryResultSchema.getFields().get(sourceTableSchema.getColumnName());
            boolean dataTypesMatch = sourceTableBQDataType.equals(sourceQueryField.getType());
            if (!dataTypesMatch) {
                foundError = true;
                LOGGER.info("Data type mismatch found for attribute {}: entity declared {}, SQL schema returns {}", attribute.getName(), sourceTableBQDataType, sourceQueryField.getType());
            }
        }
        if (foundError) {
