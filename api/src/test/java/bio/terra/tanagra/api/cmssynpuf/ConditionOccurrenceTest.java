...
public class ConditionOccurrenceTest extends BaseQueryTest {
  @Test
  void diabetes() throws IOException {
    ...
    // i.e. give me all the condition occurrence rows for people with "Type 2 diabetes mellitus".
    // this set of rows will include non-diabetes condition occurrences, such as covid.
    RelationshipFilter occurrencesOfPeopleWhoHadDiabetes =
        new RelationshipFilter(getEntity(), personOccurrenceRelationship, peopleWhoHadDiabetes);
    ...
    EntityQueryRequest entityQueryRequest =
        new EntityQueryRequest.Builder()
            ...
            .filter(occurrencesOfPeopleWhoHadDiabetes)
            .limit(DEFAULT_LIMIT)
            .build();
    GeneratedSqlUtils.checkMatchesOrOverwriteGoldenFile(
        querysService.buildInstancesQuery(entityQueryRequest).getSql(),
        "sql/cmssynpuf/conditionoccurrence-diabetes.sql");
  }
  ...
  void diabetesAndSepsis() throws IOException {
    ...
    GeneratedSqlUtils.checkMatchesOrOverwriteGoldenFile(
        querysService.buildInstancesQuery(entityQueryRequest).getSql(),
        "sql/cmssynpuf/conditionoccurrence-diabetesAndSepsis.sql");
  }
