...
public final class ComputeAncestorDescendant extends WorkflowCommand {

  private ComputeAncestorDescendant(
      String command, String description, Map<String, String> queryInputs) {
    super(command, description, queryInputs);
  }

  public static ComputeAncestorDescendant forHierarchy(Entity entity, String hierarchyName) {
    ...
    String template =
        "./gradlew workflow:execute -DmainClass=bio.terra.tanagra.workflow.FlattenHierarchy "
    ...
    String description =
        entity.getName() + " (Hierarchy = " + hierarchyName + "): ComputeAncestorDescendant";

    return new ComputeAncestorDescendant(command, description, queryInputs);
  }
}
