...
JOIN `broad-tanagra-dev.synpuf.concept` cparent  ON cparent.concept_id = cr.concept_id_1
JOIN `broad-tanagra-dev.synpuf.concept` cchild  ON cchild.concept_id = cr.concept_id_2
WHERE
  cr.relationship_id = 'Subsumes' AND
  cparent.domain_id = cchild.domain_id
;
