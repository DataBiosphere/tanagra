...
When true, the service expects a JWT, which may be unverified. The service decides the decodes the user information from the payload. When this flag is set, opt to verify the token by setting [Issuer](#tanagraauthaccessTokenissuer), [Public key file](#tanagraauthaccessTokenpublicKeyFile). [Algorithm](#tanagraauthaccessTokenalgorithm) defaults to RSA256. 
