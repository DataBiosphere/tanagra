...
## The script assumes that it is being run from the project-level directory "tanagra/".
if [ $(basename $PWD) != 'tanagra' ]; then
  echo "Script must be run from project-level directory 'tanagra/'"
  exit 1
fi
...
# used for querying source tables and generating new index tables in the broad-tanagra-dev project
