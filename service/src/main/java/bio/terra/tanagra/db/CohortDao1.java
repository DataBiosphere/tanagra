...
@Component
public class CohortDao1 {
  private static final Logger LOGGER = LoggerFactory.getLogger(CohortDao1.class);
  ...
  private static final RowMapper<CohortV1.Builder> COHORT_ROW_MAPPER =
      (rs, rowNum) ->
          CohortV1.builder()
  ...
  private static final RowMapper<CriteriaGroupV1.Builder> CRITERIA_GROUP_ROW_MAPPER =
      (rs, rowNum) ->
          CriteriaGroupV1.builder()
              .cohortId(rs.getString("cohort_id"))
  ...
  static final RowMapper<CriteriaV1> CRITERIA_ROW_MAPPER =
      (rs, rowNum) ->
          CriteriaV1.builder()
              .criteriaGroupId(rs.getString("criteria_group_id"))
  ...
  @Autowired
  public CohortDao1(NamedParameterJdbcTemplate jdbcTemplate) {
    this.jdbcTemplate = jdbcTemplate;
  }

  /** Fetch all cohorts that are the most recent version for their user-facing id. */
  @ReadTransaction
  public List<CohortV1> getAllCohortsLatestVersion(String studyId, int offset, int limit) {
   ...
   */
  @ReadTransaction
  public List<CohortV1> getCohortsMatchingListLatestVersion(
   ...
   */
  public List<CohortV1> getCohortsMatchingList(String studyId, Set<String> cohortIdList) {
  ...
  private List<CohortV1> getCohortsHelper(
      String studyId,
      int offset,
      int limit,
    ...
    List<CohortV1.Builder> cohorts = jdbcTemplate.query(sql.toString(), params, COHORT_ROW_MAPPER);
    populateCriteriaGroups(cohorts);
    return cohorts.stream().map(CohortV1.Builder::build).collect(Collectors.toList());
  }

  /** Fetch the most recent version for the given revision group id. */
  @ReadTransaction
  public CohortV1 getCohortLatestVersion(String studyId, String cohortRevisionGroupId) {
   ...
   */
  public CohortV1 getCohortVersionOrThrow(String studyId, String cohortId) {
   ...
   */
  public CohortV1 getCohortLatestVersionOrThrow(String studyId, String cohortRevisionGroupId) {
  ...
  @SuppressWarnings("PMD.InsufficientStringBufferDeclaration")
  private Optional<CohortV1> getCohortIfExistsHelper(
      String studyId,
  ...
  private void populateCriteriaGroups(List<CohortV1.Builder> cohorts) {
    if (cohorts.isEmpty()) {
      return;
    }
    ...
    MapSqlParameterSource params =
        new MapSqlParameterSource()
            .addValue(
                "cohort_ids",
                cohorts.stream().map(CohortV1.Builder::getCohortId).collect(Collectors.toList()));
    List<CriteriaGroupV1.Builder> criteriaGroups =
        ...
        new MapSqlParameterSource()
            .addValue(
                ...
                criteriaGroups.stream()
                    .map(CriteriaGroupV1.Builder::getCriteriaGroupId)
    ...
    criteriaGroups.stream()
        .forEach(
            criteriaGroupBuilder -> {
              CriteriaGroupV1 criteriaGroup = criteriaGroupBuilder.build();
              cohorts.stream()
  ...
  @WriteTransaction
  public void createCohortFirstVersion(CohortV1 cohort) {
    Optional<CohortV1> existingCohort =
  ...
  public boolean updateCohortLatestVersion(
      ...
      @Nullable List<CriteriaGroupV1> criteriaGroups) {
    CohortV1 existingCohort = getCohortLatestVersionOrThrow(studyId, cohortRevisionGroupId);
    if (existingCohort.isEditable()) {
      ...
      CohortV1.Builder newCohortVersion = existingCohort.toBuilder();
      if (displayName != null) {
  ...
  private void createCohortHelper(CohortV1 cohort) {
  ...
  private boolean insertCriteriaGroups(
      String studyId, String cohortId, List<CriteriaGroupV1> criteriaGroups) {
    ...
    for (CriteriaGroupV1 criteriaGroup : criteriaGroups) {
      paramSets.addAll(
